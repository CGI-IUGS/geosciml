...
xmlns:mef="http://www.mddep.gouv.qc.ca/sih"
>
	...
	<wfs:Query typeName="mef:WaterWell" srsName="{$s}">
	...
	<ogc:PropertyIsEqualTo>
		<ogc:PropertyName>mef:guid</ogc:PropertyName>
		<ogc:Literal><xsl:value-of select="substring-after(@gml:id,'qc.waterwell.')"/></ogc:Literal>
		</ogc:PropertyIsEqualTo>
	...
	<ogc:PropertyName>
	<xsl:choose>
		<xsl:when test=".='gwml:wellLocation'">mef:geom</xsl:when>
		<xsl:when test=".='sa:length'">mef:length</xsl:when>
		<xsl:when test=".='gwml:referenceElevation'">mef:elevation</xsl:when>
		<!-- add name + codespace 2 version -->
		<xsl:when test=".='gml:name'">mef:guid</xsl:when>
...
<xsl:template match="text()">
	...
	<!-- this relies on the fact that this id has a unique prefix, and can only match, and always do,  an id on the private side which is equivalent to the suffix, if the user had put it on the right context of not cannot be determined -->
		<xsl:when test="starts-with(.,'urn:x-ngwd:qc:mef:well_')"><xsl:value-of select="substring-after(.,'urn:x-ngwd:qc:mef:well_')"/></xsl:when>
		<xsl:otherwise><xsl:value-of select="."/></xsl:otherwise>
	</xsl:choose>
	</xsl:template>	
	<xsl:template match="@*|node()">
	<xsl:copy><xsl:apply-templates select="@*|child::node()"/></xsl:copy>
	</xsl:template>
